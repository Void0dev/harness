...
- Apply with the explicit `--write` mode. It writes canonical schema v2 plus six canonical workflows (`ci.yml`, `coolify-deploy.yml`, `backend-prepare.yml`, `coolify-rollback.yml`, `bootstrap-deployment-evidence.yml`, `evidence-retention-checkpoint.yml`) under a repository-local lock using per-file atomic replacements. A durable `.harness/write-journal-v1.json` captures originals before replacement; the next writer recovers an interrupted prepared transaction and reports `recoveredInterruptedWrite`.
